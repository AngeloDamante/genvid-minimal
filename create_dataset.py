...
def parse_instructions(istr_file: str) -> (int, int, list):
    ...
    for i in range(1, len(route_readed), 1):
        ...
        if tp == 'linear':
            ...
            logging.error("Unable to parse step type '{}', use linear|acc,|dec|trap")
            exit(6)
...
def parse_json(fn) -> list:
    ...
    for i, obj_info in enumerate(json_file):
        ...
        patch_path = check_exists_with_default_dir(obj_info["patch"], "patches")
        ...
        patch = cv2.resize(p, dsize=(int(p.shape[0] * ratio), int(p.shape[1] * ratio)), interpolation=cv2.INTER_AREA)
        ox, oy, route = parse_instructions(route_path)

        inst = Instruction(label=label, patch=patch, origin_x=ox, origin_y=oy, route=route)
